...
def create_classifiers(hparams, n_splits_outer, n_splits_inner):
    ...
    svc_params = {"random_state": [0], "cache_size": [1000]}
...
def main(hparams):
    ...
    measures_N = measures_N[["sample", "diagnose_grouped", *feature_selection_agg]]
    measures_T = measures_T[["sample", "diagnose_grouped", *feature_selection_agg]]
    measures_N["sample"] = measures_N["sample"]+1000

    measures = pd.concat([measures_N, measures_T])
    measures["split"] = ""
...
if __name__ == '__main__':
    ...
    parser.add_argument("--results_folder_name", type=str, default="train_both", help="Folder name for all files written.")
    ...
    hparams.features_folder = Path(__file__).parent.joinpath("..", "datasets", "both")
